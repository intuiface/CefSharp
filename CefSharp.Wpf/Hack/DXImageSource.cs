...
namespace SharpDX.WPF
{
	public class DXImageSource : D3DImage, IDisposable
	{
        ...
        private void DXImageSource_IsFrontBufferAvailableChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            if (this.IsFrontBufferAvailable == false)
            {
                UpState = false;
            }
            if (this.IsFrontBufferAvailable == true && UpState == false)
            {
                if (OnContextRetreived != null)
                {
                    OnContextRetreived(sender, null);
                }
            }
        }

        public event EventHandler OnContextRetreived;
